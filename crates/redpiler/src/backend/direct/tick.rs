...
use super::{
	DirectBackend, NodeType, TickPriority, calculate_comparator_output, get_all_input,
	get_bool_input, node::NodeId, schedule_tick,
};
...
impl DirectBackend {
	pub fn tick_node(&mut self, node_id: NodeId) {
		...
		match node.ty {
			NodeType::Repeater { delay, .. } if !node.locked => {
			...
			NodeType::Torch => {
				...
				if node.powered != should_be_powered {
					self.set_node(
						node_id,
						should_be_powered,
						bool_to_signal_strength(should_be_powered),
					);
				}
			}
			NodeType::Comparator {
			...
			NodeType::Button if node.powered => self.set_node(node_id, false, 0),
			_ => {}
		}
	}
}
