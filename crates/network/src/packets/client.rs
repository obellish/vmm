...
impl ClientBoundPacket for CResponse {
	fn encode(&self) -> PacketEncoder {
		let mut buf = Vec::new();
		buf.write_identifier(&self.json_response);
...
impl ClientBoundPacket for CDisconnectLogin {
	fn encode(&self) -> PacketEncoder {
		let mut buf = Vec::new();
		buf.write_identifier(&self.reason);
		PacketEncoder::new(buf, 0x00)
	}
}
...
impl CRegistryDataCodec {
	fn encode(&self, buf: &mut Vec<u8>) {
		let mut dimension_map = NbtMap::new("minecraft:dimension_type");
		for (name, element) in &self.dimension_types {
			dimension_map.push_element(name.clone(), element.clone());
		}

		let mut biome_map = NbtMap::new("minecraft:worldgen/biome");
		...
		let mut damage_map = NbtMap::new("minecraft:damage_type");
		for ty in required_types {
			damage_map.push_element(
				format!("minecraft:{ty}"),
				CRegistryDamageType {
					message_id: "generic".to_owned(),
					scaling: "always".to_owned(),
					..Default::default()
				},
			);
		}
...
impl ClientBoundPacket for CSpawnEntity {
	fn encode(&self) -> PacketEncoder {
		...
		buf.write_byte(((self.pitch / 360.0 * 256.0) as i32 % 256) as i8);
		buf.write_byte(((self.yaw / 360.0 * 256.0) as i32 % 256) as i8);
		buf.write_byte(((self.head_yaw / 360.0 * 256.0) as i32 % 256) as i8);
	...
	fn encode(&self) -> PacketEncoder {
		...
		for node in &self.nodes {
			...
			if let Some(suggesstions_type) = node.suggestions_type {
				buf.write_identifier(suggesstions_type);
			}
		}
...
impl ClientBoundPacket for CSetContainerContent {
	fn encode(&self) -> PacketEncoder {
		...
		for slot_data in &self.slot_data {
			buf.write_slot_data(slot_data.as_ref());
		}
		buf.write_slot_data(self.carried_item.as_ref());
...
impl ClientBoundPacket for CSetContainerSlot {
	fn encode(&self) -> PacketEncoder {
		...
		buf.write_slot_data(self.slot_data.as_ref());
	...
	fn encode(&self) -> PacketEncoder {
		...
		for chunk_section in &self.chunk_sections {
			...
			for container in containers {
				...
				if matches!(container.bits_per_entry, 0) {
...
impl CPlayerInfoActions {
	...
	fn encode<T: PacketEncoderExt>(&self, buf: &mut T) {
...
impl ClientBoundPacket for CPlayerInfoUpdate {
	fn encode(&self) -> PacketEncoder {
		...
		let mask = self
			.players
			.first()
			.map_or(0, |player| player.actions.mask());
...
impl ClientBoundPacket for CUpdateSectionBlocks {
	fn encode(&self) -> PacketEncoder {
		...
		let pos = ((i64::from(self.chunk_x) & 0x003F_FFFF) << 42)
			| ((i64::from(self.chunk_z) & 0x003F_FFFF) << 20)
			| (i64::from(self.chunk_y) & 0xFFFFF);
		buf.write_long(pos);
		buf.write_varint(self.records.len() as i32);
		for record in &self.records {
			let long = (u64::from(record.block_id) << 12)
				| (u64::from(record.x) << 8)
				| (u64::from(record.z) << 4)
				| u64::from(record.y);
...
impl ClientBoundPacket for CDisplayObjective {
	fn encode(&self) -> PacketEncoder {
		...
		buf.write_identifier(&self.score_name);
...
impl ClientBoundPacket for CSetEquipment {
	fn encode(&self) -> PacketEncoder {
		...
		for slot in &self.equipment {
			...
			buf.write_slot_data(slot.item.as_ref());
		}
...
impl ClientBoundPacket for CUpdateScore {
	fn encode(&self) -> PacketEncoder {
		let mut buf = Vec::new();
		buf.write_identifier(&self.entity_name);
		buf.write_identifier(&self.objective_name);
...
impl ClientBoundPacket for CUpdateTime {
	fn encode(&self) -> PacketEncoder {
		let mut buf = Vec::new();
...
impl CDeclareCommandsNodeParser {
	fn write(&self, buf: &mut Vec<u8>) {
		match self {
			...
			Self::Vec2 => buf.write_varint(11),
			Self::Vec3 => buf.write_varint(10),
			Self::BlockPos => buf.write_varint(8),
			Self::BlockState => buf.write_varint(12),
