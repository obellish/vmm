...
impl ServerBoundPacket for SLoginPluginResponse {
		...
		Self: Sized,
	{
		Ok(Self {
			...
			data: PacketDecoderExt::read_to_end(reader)?,
		})
	}
...
impl ServerBoundPacket for SChatCommand {
		...
		Self: Sized,
	{
		...
		let timestamp = reader.read_long()?;
		let salt = reader.read_long()?;
		let mut argument_signatures = Vec::new();
		let num_signatures = reader.read_varint()?;
		for _ in 0..num_signatures {
			argument_signatures.push(SChatCommandArgumentSignature {
				argument_name: reader.read_string()?,
				signature: reader.read_bytes(256)?,
			});
		}

		let message_count = reader.read_varint()?;
		let acknowledged = reader.read_bytes(3)?.try_into().unwrap();
...
impl ServerBoundPacket for SPlayerAbilities {
	...
	{
		Ok(Self {
			is_flying: !matches!(reader.read_byte()?, 0),
		})
	}
...
impl ServerBoundPacket for SSetCreativeModeSlot {
		...
		Self: Sized,
	{
		...
		let clicked_item = if reader.read_bool()? {
			Some(SlotData {
...
impl ServerBoundPacket for SUpdateSign {
		...
		Self: Sized,
	{
		...
		let is_front_text = reader.read_bool()?;
		let lines = [
			reader.read_string()?,
			reader.read_string()?,
			reader.read_string()?,
...
pub trait ServerBoundPacket: Debug + Send {
	fn decode<T: PacketDecoderExt>(reader: &mut T) -> DecodeResult<Self>
	where
		Self: Sized;
