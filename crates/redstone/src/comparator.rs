...
use vmm_blocks::{
	...
	blocks::{Block, ComparatorMode, RedstoneComparator, entities::BlockEntity},
};
use vmm_world::{TickPriority, World};

#[must_use]
pub const fn has_override(block: Block) -> bool {
	matches!(
		block,
		Block::Barrel {}
			| Block::Furnace {}
...
pub fn get_override(block: Block, world: &impl World, pos: BlockPos) -> u8 {
	match block {
		...
		Block::Cauldron { level } | Block::Composter { level } => level,
...
pub fn should_be_powered(comp: RedstoneComparator, world: &impl World, pos: BlockPos) -> bool {
	...
	if matches!(input_strength, 0) {
		...
		if input_strength > power_on_sides {
			...
			power_on_sides == input_strength && matches!(comp.mode, ComparatorMode::Compare)
		}
	}
}

pub fn get_far_input(world: &impl World, pos: BlockPos, facing: BlockDirection) -> Option<u8> {
	let face = BlockFace::from(facing);
...
pub fn update(comp: RedstoneComparator, world: &mut impl World, pos: BlockPos) {
	...
	if output_strength != old_strength || comp.powered != should_be_powered(comp, world, pos) {
		let front_block = world.get_block(pos.offset((!comp.facing).into()));
...
pub fn tick(mut comp: RedstoneComparator, world: &mut impl World, pos: BlockPos) {
	...
	if new_strength != old_strength || matches!(comp.mode, ComparatorMode::Compare) {
...
fn get_power_on_side(world: &impl World, pos: BlockPos, side: BlockDirection) -> u8 {
	...
	if super::is_diode(side_block) {
		...
		wire.power
	} else if matches!(side_block, Block::RedstoneBlock {}) {
		15
	} else {
		0
	}
...
fn get_power_on_sides(comp: RedstoneComparator, world: &impl World, pos: BlockPos) -> u8 {
	std::cmp::max(
		get_power_on_side(world, pos, comp.facing.rotate_cw()),
...
fn calculate_output_strength(comp: RedstoneComparator, world: &impl World, pos: BlockPos) -> u8 {
	let input_strength = calculate_input_strength(comp, world, pos);
	if matches!(comp.mode, ComparatorMode::Subtract) {
...
fn on_state_change(comp: RedstoneComparator, world: &mut impl World, pos: BlockPos) {
	let front_pos = pos.offset((!comp.facing).into());
