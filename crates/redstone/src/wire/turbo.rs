...
impl RedstoneWireTurbo {
	...
	fn new() -> Self {
		Self {
			...
			update_queue: vec![Vec::new(), Vec::new(), Vec::new()],
	...
	fn identify_neighbors(&mut self, world: &impl World, upd1: NodeId) {
		...
		let UpdateNode { xbias, zbias, .. } = self.get_node(upd1);
		let xbias = *xbias;
		let zbias = *zbias;

		let heading;
		if matches!((cx, cz), (0, 0)) {
	...
	fn propagate_changes(&mut self, world: &impl World, upd1: NodeId, layer: u32) {
		if self.nodes[upd1.index].neighbors.is_some() {
	...
	fn update_node(&mut self, world: &mut impl World, upd1: NodeId, layer: u32) {
		...
		let new_wire = self.calculate_current_changed(world, upd1);
	...
	fn calculate_current_changed(&mut self, world: &mut impl World, upd: NodeId) -> RedstoneWire {
...
impl UpdateNode {
	fn new(world: &impl World, pos: BlockPos) -> Self {
		Self {
			pos,
...
const fn wire_mut(block: &mut Block) -> &mut RedstoneWire {
	match block {
...
const fn unwrap_wire(block: Block) -> RedstoneWire {
	match block {
		Block::RedstoneWire { wire } => wire,
		_ => panic!("expected wire"),
	}
