...
impl LibraryNamespace {
	...
	pub fn new(source: impl AsRef<str>) -> Result<Self, LibraryNamespaceError> {
		source.as_ref().parse()
	}
	...
	pub fn validate(source: impl AsRef<str>) -> Result<(), LibraryNamespaceError> {
		...
		if matches!(
			source,
			Self::KERNEL_PATH | Self::EXEC_PATH | Self::ANON_PATH
		) {
			return Ok(());
		}
...
impl Display for LibraryNamespace {
...
impl Serializable for LibraryNamespace {
	fn write_into<W: ByteWriter>(&self, target: &mut W) {
		...
		assert!(bytes.len() <= Self::MAX_LENGTH, "namespace too long");
