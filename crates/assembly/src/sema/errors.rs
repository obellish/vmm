...
pub enum SemanticAnalysisError {
	...
	#[error("invalid module: conflicting entrypoints defined")]
...
impl Display for LimitKind {
	fn fmt(&self, f: &mut Formatter<'_>) -> FmtResult {
		f.write_str(match self {
			Self::Procedures => "too many procedures in module",
			Self::Locals => "too many procedure locals",
			Self::Imports => "too many imported procedures",
			Self::CalledImports => "too many calls to imported procedures",
			Self::Instructions => "too many instructions in block",
		})
	}
}
