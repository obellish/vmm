	...
	C: Clone + Ord,
{
	...
	kruskal_indices(nodes.len(), edges).filter_map(move |(ia, ib, w)| {
		Some((
			<&N>::clone(nodes.get_index(ia)?),
			<&N>::clone(nodes.get_index(ib)?),
			w,
		))
	})
}
...
mod tests {
	...
	#[test]
	fn path_halving() {
		let mut parents = [0, 0, 1, 2, 3, 4, 5, 6];
		assert_eq!(find(&mut parents, 7), 0);
		assert_eq!(parents, [0, 0, 1, 1, 3, 3, 5, 5]);
		assert_eq!(find(&mut parents, 7), 0);
		assert_eq!(parents, [0, 0, 1, 0, 3, 3, 5, 3]);
		assert_eq!(find(&mut parents, 7), 0);
		assert_eq!(parents, [0, 0, 1, 0, 3, 3, 5, 0]);
		assert_eq!(find(&mut parents, 6), 0);
		assert_eq!(parents, [0, 0, 1, 0, 3, 3, 3, 0]);
		assert_eq!(find(&mut parents, 6), 0);
		assert_eq!(parents, [0, 0, 1, 0, 3, 3, 0, 0]);
	}
}
