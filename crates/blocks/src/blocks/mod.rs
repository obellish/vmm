...
use vmm_derive::BlockTransform;

pub use self::props::*;
use super::{BlockColorVariant, BlockDirection, BlockFacing, BlockProperty, SignType};
...
trait BlockTransform {
	...
	fn rotate(&mut self, amt: RotateAmt) {
		match amt {
...
impl BlockTransform for BlockDirection {
	fn flip(&mut self, dir: FlipDirection) {
		match dir {
			FlipDirection::FlipX => {
				if let t @ (Self::East | Self::West) = self {
					*t = !*t;
				}
			}
			FlipDirection::FlipZ => {
				if let t @ (Self::North | Self::South) = self {
					*t = !*t;
				}
			}
		}
...
macro_rules! impl_block_transform {
	($($ty:ty),*$(,)?) => {
		$(
			impl $crate::blocks::BlockTransform for $ty {
				fn rotate90(&mut self) {}

				fn flip(&mut self, _: $crate::blocks::FlipDirection) {}
			}
		)*
	}
}

impl_block_transform!(
	u8,
	u32,
	bool,
	BlockColorVariant,
                ...
                match self {
                    $(
                        Block::$name {
                            ...
                            $(
                                <$prop_type as BlockProperty>::decode($prop_name, stringify!($prop_name), &props);
                            )*
                        },
                    )*
                }
                ...
                match self {
                    $(
                        Block::$name {
                            ...
                            $(
                                <$prop_type as BlockProperty>::encode(*$prop_name, stringify!($prop_name), &mut props);
                            )*
                        },
                    )*
                }
...
blocks! {
	...
	RedstoneWire {
		...
		from_id(id): 2978..=4273 => {
			wire: RedstoneWire::new(
				RedstoneWireSide::from_id(id % 432 / 144).unwrap(),
				RedstoneWireSide::from_id(id % 9 / 3).unwrap(),
				RedstoneWireSide::from_id(id / 432).unwrap(),
				RedstoneWireSide::from_id(id % 3).unwrap(),
				(id % 144 / 9) as u8,
			)
		},
		from_names(_name): {
	...
	WallSign {
		...
		get_id: (sign_type.0 << 3) + (facing.id() << 1) + match sign_type.0 {
			0..=5 => 4763,
			6..=7 => 19341 - (6 << 3),
			_ => unreachable!(),
		},
	...
	Lever {
		...
		get_id: {
			(lever.face.id() << 3)
				+ (lever.facing.id() << 1)
				+ u32::from(!lever.powered)
				+ 5626
		},
		from_id_offset: 5626,
		from_id(id): 5626..=5649 => {
			lever: Lever::new(
				LeverFace::from_id(id >> 3).unwrap(),
				BlockDirection::from_id((id >> 1) & 0b11).unwrap(),
				(id & 1) == 0
			)
		},
		from_names(_name): {
	...
	StoneButton {
		...
		get_id: {
			(button.face.id() << 3)
				+ (button.facing.id() << 1)
				+ u32::from(!button.powered)
				+ 5748
		},
		from_id_offset: 5748,
		from_id(id): 5748..=5771=> {
			button: StoneButton::new(ButtonFace::from_id(id >> 3).unwrap(), BlockDirection::from_id((id >> 1) & 0b11).unwrap(), (id & 1) == 0)
		},
		from_names(_name): {
			"stone_button" => {
				button: StoneButton::default()
			}
		},
	...
	RedstoneWallTorch {
		...
		get_id: (facing.id() << 1) + u32::from(!lit) + 5740,
		from_id_offset: 5740,
		from_id(id): 5740..=5747 => {
			lit: (id & 1) == 0,
			facing: BlockDirection::from_id(id >> 1).unwrap()
		},
		from_names(_name): {
			"redstone_wall_torch" => {
				lit: true,
	...
	RedstoneRepeater {
		...
		get_id: {
			(u32::from(repeater.delay) - 1) * 16
				+ repeater.facing.id() * 4
				+ u32::from(!repeater.locked) * 2
				+ u32::from(!repeater.powered)
				+ 5881
		},
		from_id_offset: 5881,
		from_id(id): 5881..=5944 => {
			repeater: RedstoneRepeater::new(
				(id >> 4) as u8 + 1,
				BlockDirection::from_id((id >> 2) & 3).unwrap(),
				((id >> 1) & 1) == 0,
				(id & 1) == 0
			)
		},
	...
	TripwireHook {
		...
		from_id(id): 7530..=7536 => {
			direction: BlockDirection::from_id(id / 2).unwrap()
		},
		from_names(_name): {
			"tripwire_hook" => {
	...
	RedstoneComparator {
		...
		get_id: {
			comparator.facing.id() * 4
				+ comparator.mode.id() * 2
				+ u32::from(!comparator.powered)
				+ 9175
		},
		from_id_offset: 9175,
		from_id(id): 9175..=9190 => {
			comparator: RedstoneComparator::new(
				BlockDirection::from_id(id >> 2).unwrap(),
				ComparatorMode::from_id((id >> 1) & 1).unwrap(),
				(id & 1) == 0
			)
		},
		from_names(_name): {
			"comparator" => {
				comparator: RedstoneComparator::default()
			}
		},
		get_name: "comparator",
	},
	...
	Observer {
		...
		get_id: (facing.id() << 1) + 12551,
		from_id_offset: 12551,
		from_id(id): 12551..=12561 => {
			facing: BlockFacing::from_id(id >> 1).unwrap()
		},
		from_names(_name): {
			"observer" => {
	...
	SeaPickle {
		...
		get_id: u32::from((pickles - 1) << 1) + 12934,
	...
	IronTrapdoor {
		...
		from_id(id): 10400..=10462 => {
			facing: BlockDirection::from_id(id >> 4).unwrap(),
			half: TrapdoorHalf::from_id((id >> 3) & 1).unwrap(),
			powered: ((id >> 1) & 1) == 0
		},
		from_names(_name): {
			"iron_trapdoor" => {
	...
	NoteBlock {
		...
		from_id_offset: 538,
		from_id(id): 538..=1637 => {
			instrument: Instrument::from_id((id >> 1) / 25).unwrap(),
			note: (id >> 1) % 25,
			powered: (id & 1) == 0
		},
		from_names(_name): {
