...
impl<T, F> ExactSizeIterator for SortedBy<T, F> where F: FnMut(&T, &T) -> Ordering {}
...
impl<T, K: Ord, F> ExactSizeIterator for SortedByKey<T, K, F> where F: FnMut(&T) -> K {}
...
mod tests {
	...
	fn arbitrary_sorting() {
		run_test(|u| {
			...
			let actually_sorted = value.into_iter().sorted().collect::<Vec<_>>();

			check_sorting(&actually_sorted);
