...
impl<const CNT: usize> Droppables<CNT> {
	fn test(value_idx: usize, value: u32)  -> Result<() >{
        ...
        let mut zero_to_cnt_set = (0..CNT as u32).map(|v| v * 3).into_iter().collect::<HashSet<_>>();
        ...
        let zero_to_value_idx_set = (0..value_idx as u32).map(|v| v * 3).into_iter().collect::<HashSet<_>>();
