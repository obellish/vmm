...
#[cfg(test)]
mod tests {
	use super::{Deserializable, DeserializationError, FeltRng, RpxRandomCoin, Serializable, ZERO};
	use crate::ONE;
	...
	fn feltrng_serialization() -> Result<(), DeserializationError> {
		...
		let coin2 = RpxRandomCoin::read_from_bytes(&bytes)?;
